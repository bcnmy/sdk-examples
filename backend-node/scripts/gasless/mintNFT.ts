...
export const mintNft = async () => {
  ...
  const bundler = new Bundler({
    ...
    entryPointAddress: "0x29d4cfa9869c4fb4a78a6f7f32468f5e0b78da4e"
  });
  ...
  try {
    ...
    const paymasterResponse = await paymaster.getPaymasterAndData(
      partialUserOp,
      {
    ...
    partialUserOp.paymasterAndData = paymasterResponse.paymasterAndData;
    partialUserOp.callGasLimit = paymasterResponse.callGasLimit;
    partialUserOp.verificationGasLimit = paymasterResponse.verificationGasLimit;
    partialUserOp.preVerificationGas = paymasterResponse.preVerificationGas;

    console.time("sendUserOp");
    let userOpResponse = await biconomySmartAccount.sendUserOp(partialUserOp);
